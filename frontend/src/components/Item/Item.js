...
function Item(props) {
  ...
  const onNavigateItemPage = () => {
    navigate("/item/" + props.item.item_ID, {
      state: { item_ID: props.item.item_ID },
    });
  };
    ...
    <div className="hover">
      ...
      <CCard className={`mb-3 border-light border-top-3 border-top-light`}>
        ...
        <CCardBody>
          ...
          <CCardText>
            Stock:
            {props.item.quantity > 0 ? props.item.quantity : "Out of Stock"}
          </CCardText>
          <CCardText>
            {currency}
            {props.item.price}
